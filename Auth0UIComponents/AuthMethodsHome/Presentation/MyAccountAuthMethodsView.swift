...
struct MyAccountAuthMethodsView: View {
    @StateObject private var navigationStore =  NavigationStore.shared
    @ObservedObject var viewModel: MyAccountAuthMethodsViewModel

    init(viewModel: MyAccountAuthMethodsViewModel) {
    ...
    @ViewBuilder
    func authMethodView(_ component: MyAccountAuthViewComponentData) -> some View {
        switch component {
        case .title(let text):
            Text(text)
    ...
    func handleRoute(route: Route) -> some View {
        ...
        case let .otpScreen(type, emailOrPhoneNumber, totpEnrollmentChallege, phoneEnrollmentChallenge, emailEnrollmentChallenge):
            OTPView(viewModel: OTPViewModel(totpEnrollmentChallenge: totpEnrollmentChallege, emailEnrollmentChallenge: emailEnrollmentChallenge, phoneEnrollmentChallenge: phoneEnrollmentChallenge, type: type, emailOrPhoneNumber: emailOrPhoneNumber))
        case let .filteredAuthListScreen(type, authMethods):
            SavedAuthenticatorsScreen(viewModel: SavedAuthenticatorsScreenViewModel(type: type, authenticationMethods: authMethods))
